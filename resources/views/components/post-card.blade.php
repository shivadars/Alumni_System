...
<div x-data="{ 
    showComments: false,
    comments: {{ Js::from($post->comments->take(5)->map(function($comment) {
        return [
            'id' => $comment->id,
...
}" class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all duration-300">
    ...
    <div x-data="{ 
        ...
        <button @click="showComments = !showComments" class="flex items-center gap-2 hover:text-blue-600 transition-colors px-1" :class="{ 'text-blue-600': showComments }">
            ...
            <span class="font-bold text-sm" x-text="comments.length"></span>
        </button>
    ...
    <div x-show="showComments" style="display: none;" x-transition class="mt-8 pt-8 border-t border-slate-50">
        ...
        <div class="space-y-6 mb-8">
            <template x-for="comment in comments" :key="comment.id">
                <div class="flex gap-4">
                    <template x-if="comment.profile_picture">
                        <img :src="comment.profile_picture" :alt="comment.user.name" class="w-8 h-8 rounded-full object-cover shadow-sm">
                    </template>
                    <template x-if="!comment.profile_picture">
                    ...
                    <div class="flex-1">
                        <div class="bg-slate-50/50 p-4 rounded-2xl border border-slate-100/50">
                            <div class="flex items-start justify-between gap-4">
                                <div class="flex-1">
                                    <a :href="'{{ url('profile') }}/' + comment.user.id" class="font-extrabold text-slate-900 text-[13px] mb-1 leading-tight hover:text-blue-600 transition-colors" x-text="comment.user.name"></a>
                                    <p class="text-slate-600 text-[13px] leading-relaxed" x-text="comment.content"></p>
                                </div>
                                <template x-if="comment.user_id === authUserId || authUserRole === 'admin'">
                                    <button @click="deleteComment(comment.id)" class="p-1 text-slate-300 hover:text-rose-500 transition-colors" title="Delete Comment">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                    </button>
                                </template>
                            </div>
                        </div>
                        <p class="text-[9px] text-slate-400 font-black mt-2 ml-2 tracking-widest uppercase" x-text="comment.created_at_human"></p>
                    </div>
                </div>
            </template>
        ...
        <!-- Comment Form -->
        <form @submit.prevent="submitComment" class="flex gap-4 mt-8">
            <div class="flex-1 relative">
                <textarea x-model="newComment" name="content" rows="1" class="w-full bg-slate-50/50 rounded-2xl border-slate-100 text-[13px] focus:ring-4 focus:ring-blue-50 focus:border-blue-400 focus:bg-white placeholder-slate-400 py-3.5 px-4 transition-all duration-300" placeholder="Write a comment..." required></textarea>
                <button type="submit" :disabled="isSubmitting" class="absolute right-2 top-2 p-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all duration-300 shadow-md shadow-blue-100 group disabled:opacity-50">
                    <svg class="w-4 h-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" :class="{ 'animate-spin': isSubmitting }">
                        <path x-show="!isSubmitting" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path>
                        <path x-show="isSubmitting" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4h5v.5L4.5 8.5V9H4V4zm7 0h5v.5l-4.5 4V9h-.5V4z"></path>
                    </svg>
                </button>
            </div>
        </form>
