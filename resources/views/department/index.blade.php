        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @forelse($departments as $department)
                    <a href="{{ route('department.show', $department) }}" class="group bg-white p-8 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md hover:border-blue-200 transition-all duration-300 flex flex-col items-center text-center">
                        ...
                        <h3 class="text-xl font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{{ $department }}</h3>
                ...
                @empty
                    <div class="col-span-full bg-white p-12 text-center rounded-2xl shadow-sm border border-slate-100">
                        ...
                        <p class="text-slate-500 mt-2">There are currently no alumni associated with any departments.</p>
                    </div>
                @endforelse
            </div>
        </div>
